...
function handleLocationError(err) {
  ...
  switch (err.code) {
    case err.PERMISSION_DENIED:
      message = "Location permission denied. Enable it in your browser/site settings to see your position on the map.";
      break;
    case err.POSITION_UNAVAILABLE:
      message = "Location unavailable right now — try again in an open area.";
      break;
    case err.TIMEOUT:
      message = "Location request timed out. Tap the locate button to retry.";
      break;
  }
  console.warn("Geolocation error:", err.message);
  showLocationToast(message);
}
...
function showLocationToast(message) {
  ...
  clearTimeout(showLocationToast._timer);
  showLocationToast._timer = setTimeout(() => {
...
function startTrackingUserLocation() {
  if (!("geolocation" in navigator)) {
    showLocationToast("Geolocation isn't supported on this browser.");
    return;
  }

  if (!window.isSecureContext) {
    showLocationToast("Location requires HTTPS. Host this page over https:// (or localhost) to use it.");
    return;
  }
  ...
  if (!isWatchingLocation) {
    navigator.geolocation.watchPosition(updateUserLocation, handleLocationError, options);
